...
-- Get top products by units and revenue (from PAID orders only)
create or replace function public.admin_top_products(p_limit int default 5)
returns table(product_id uuid, name text, units int, revenue_cents int)
language sql security definer set search_path = public
as $$
  select public._assert_admin(); -- Call admin check first
...
begin
  ...
  raise notice '- admin_top_products() - Top products by revenue from paid orders';
