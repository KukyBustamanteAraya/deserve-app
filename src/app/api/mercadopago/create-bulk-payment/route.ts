...
export async function POST(request: Request) {
  try {
    ...
    const totalAmountCents = orders.reduce(
      (sum, o) => sum + o.total_amount_cents,
      0
    );
    ...
    if (bulkError || !bulkPayment) {
      console.error('[Bulk-Payment] Error creating bulk payment:', bulkError);
      return NextResponse.json(
    ...
    if (linkError) {
      console.error('[Bulk-Payment] Error linking orders:', linkError);
