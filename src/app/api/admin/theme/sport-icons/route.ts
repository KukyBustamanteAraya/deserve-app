...
export async function POST(request: NextRequest) {
  try {
    ...
    logger.error('Error saving sport icon settings:', toError(error));
...
export async function GET(request: NextRequest) {
  try {
    const settings = await getSettings();
    const response = NextResponse.json(settings.sportIcons || {}, { status: 200 });
    // Add cache headers to speed up repeated requests
    response.headers.set('Cache-Control', 'public, s-maxage=60, stale-while-revalidate=120');
    return response;
  } catch (error) {
    logger.error('Error getting sport icon settings:', toError(error));
    return NextResponse.json(
