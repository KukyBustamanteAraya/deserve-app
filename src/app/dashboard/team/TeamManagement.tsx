...
import { TeamCreateFormNew as TeamCreateForm } from './TeamCreateFormNew';
...
interface TeamManagementProps {
  user: any;
  profile: UserProfile;
  team: TeamDetailsType | null;
  sports?: Sport[];
  createTeamAction?: (formData: FormData) => Promise<void>;
}

export default function TeamManagement({ user, profile, team, sports = [], createTeamAction }: TeamManagementProps) {
      ...
      <div className="p-6">
        {activeTab === 'create' && (
          <div>
            ...
            {createTeamAction ? (
              <TeamCreateForm sports={sports} action={createTeamAction} />
            ) : (
              <p className="text-red-600">Unable to load team creation form. Please refresh the page.</p>
            )}
          </div>
        )}
