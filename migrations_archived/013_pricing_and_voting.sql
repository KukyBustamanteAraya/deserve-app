-- Migration: 013_pricing_and_voting.sql
...
-- Example pricing tiers (to be populated from CSV)
-- Will be seeded via script after products are created
...
-- ============================================================================
-- DESIGN CANDIDATES TABLE (for voting)
-- ============================================================================
...
CREATE TABLE IF NOT EXISTS public.design_votes (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  candidate_id UUID NOT NULL REFERENCES public.design_candidates(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS public.manufacturer_order_assignments (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  manufacturer_id UUID NOT NULL REFERENCES public.manufacturer_users(id) ON DELETE CASCADE,
...
BEGIN
  IF NEW.current_stage = 'in_production' AND OLD.current_stage != 'in_production' THEN
